...
static bool         g_driver_enabled;
static pid_t        g_keeper_pid = (pid_t)-1;
static bool         g_running;
static BldcMode     g_mode = BLDC_MODE_NONE;
...
static int bldc_encoder_init(void)
{
  struct encoder_angle_s angle;
  return TopicManager::instance().readEncoderAngle(angle) ? 0 : -ENODEV;
}

static void bldc_encoder_deinit(void)
{
}

static bool bldc_encoder_is_ready(void)
{
  struct encoder_angle_s angle;
  return TopicManager::instance().readEncoderAngle(angle);
}

static int bldc_encoder_read_raw(uint16_t *raw)
{
  struct encoder_angle_s angle;
  ...
  if (!TopicManager::instance().readEncoderAngle(angle))
    {
      return -ENODEV;
    }
...
static float bldc_elec_angle(uint16_t raw)
{
  float e = (float)g_enc_dir * (float)BOARD_BLDC_POLE_PAIRS
          * bldc_encoder_raw_to_mech_rad(raw) + g_elec_offset_rad;
...
static float bldc_meas_mech_hz(uint16_t raw, float dt)
{
  ...
  if (g_speed_delta_accum > -BLDC_SPEED_NOISE_COUNTS
   && g_speed_delta_accum < BLDC_SPEED_NOISE_COUNTS)
    ...
    {
      inst_hz = ((float)g_speed_delta_accum / (float)BLDC_ENCODER_RAW_MAX)
              / g_speed_dt_accum;
    }
...
{
  float abs_hz = __builtin_fabsf(elec_hz);
...
static float bldc_position_pd(float err_mech_rad, float dt)
{
  ...
  abs_err = __builtin_fabsf(g_err_filt_rad);

  if (abs_err < BLDC_POS_DEADBAND_RAD
      && __builtin_fabsf(g_vel_err_hz) < BLDC_POS_VEL_ZERO_HZ)
    {
  ...
  vq_lim = g_vq_max_pu * __builtin_sqrtf(__builtin_fminf(abs_err / BLDC_POS_ERR_FULL_RAD, 1.0f));
  if (vq > vq_lim)
    {
      vq = vq_lim;
    }
  ...
  g_target_mech_hz = __builtin_copysignf(vq_lim, g_err_filt_rad);
  return vq;
}
...
{
  int ret = tarox_board_control_set(TAROX_BOARD_CONTROL_BLDC_ENABLE, on);
...
static void bldc_driver_disable(void)
{
  if (g_driver_enabled)
    {
      tarox_board_control_set(TAROX_BOARD_CONTROL_BLDC_ENABLE, false);
...
{
  float mech_rad = bldc_encoder_raw_to_mech_rad(g_enc_raw);
...
static float bldc_speed_pi(float target_hz, float dt)
{
  ...
  ff = __builtin_copysignf(bldc_effective_vq(g_elec_hz, g_vq_max_pu), target_hz);
...
static void bldc_speed_control_update(float dt)
{
  ...
  if (__builtin_fabsf(g_meas_mech_hz) > BLDC_COMMUT_ADV_MIN_HZ)
    {
...
{
  const AlphaBeta voltage = Park::inverse(vd, vq, theta);
  const PhaseDuty phase_duty = Svpwm::modulate(voltage.alpha, voltage.beta);

  struct actuator_phase_duty_s duty;
  duty.timestamp = tarox_absolute_time_us();
  duty.duty_u = phase_duty.u;
  duty.duty_v = phase_duty.v;
  duty.duty_w = phase_duty.w;
  duty.armed = true;
  TopicManager::instance().publishActuatorPhaseDuty(duty);
  return 0;
}

static void bldc_publish_disarmed(void)
{
  struct actuator_phase_duty_s duty;
  duty.timestamp = tarox_absolute_time_us();
  duty.duty_u = 0.5f;
...
float Bldc::targetMechDeg()
{
  return g_target_mech_rad * 180.0f / BLDC_PI;
}

int Bldc::start(float elec_hz, float vq_pu)
{
  uint16_t raw;
  int ret;
  ...
  if (!g_running && !g_foc_calibrated)
    {
      ret = Bldc::align(BLDC_AUTO_ALIGN_VQ_PU);
      if (ret < 0)
        {
          return ret;
        }
...
int Bldc::gotoMechDeg(float mech_deg, float vq_pu)
{
  uint16_t raw;
  int ret;
  ...
  g_target_mech_rad = bldc_wrap_theta_val(mech_deg * BLDC_PI / 180.0f);

  if (g_running)
    {
      if (g_mode != BLDC_MODE_POSITION || !bldc_encoder_is_ready())
        {
          return -EINVAL;
        }
      raw = g_enc_raw;
...
int Bldc::hold(float vq_pu)
{
  ...
  if (g_running)
    {
      ...
      g_target_mech_rad = bldc_encoder_raw_to_mech_rad(raw);
      g_vq_max_pu = vq_pu;
      bldc_position_reset(raw);
      return 0;
    }
  ...
  g_target_mech_rad = bldc_encoder_raw_to_mech_rad(raw);
  return Bldc::gotoMechDeg(g_target_mech_rad * 180.0f / BLDC_PI, vq_pu);
}
...
int Bldc::setElecHz(float elec_hz)
{
...
int Bldc::encoderRead(uint16_t *raw)
{
  int ret;
...
int Bldc::align(float vq_pu)
{
  ...
  mech0 = bldc_encoder_raw_to_mech_rad(raw0);
  ...
  ret = bldc_encoder_read_raw(&raw0);
  if (ret < 0)
    {
      goto align_fail;
    }
  ...
  ret = bldc_encoder_read_raw(&raw1);
  if (ret < 0)
    {
      goto align_fail;
    }

  mech1 = bldc_encoder_raw_to_mech_rad(raw1);
  delta_mech = bldc_mech_angle_error(mech1, mech0);

  if (__builtin_fabsf(delta_mech) < 0.01f)
    {
